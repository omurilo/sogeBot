    ...
    if (!this.client) {
      ...
      this.client.on('interactionCreate', async (interaction) => {
        if (interaction.isButton()) {
          if (interaction.customId === 'ban-confirm') {
          ...
          } else if (interaction.customId === 'timeout-confirm') {
            const username = new DiscordJs.TextInputBuilder()
              .setCustomId('username-input')
              .setLabel('Nome do usuário na twitch')
              .setStyle(DiscordJs.TextInputStyle.Short)
            ...
            const rows = [username, time, reason, inputFileUrl].map(
              (component) => new DiscordJs.ActionRowBuilder<DiscordJs.TextInputBuilder | DiscordJs.StringSelectMenuBuilder>()
        ...
        if (interaction.isModalSubmit()) {
          ...
          const time = interaction.fields.getTextInputValue('time-input');
        ...
        if (commandName === 'ban-or-timeout') {
          ...
          collector?.on('end', async i => {
            if (['ban-confirm', 'timeout-confirm', 'ban-cancel'].includes(i.first()?.customId || '')) {
              const rowComponents = row.components.map(b => {
                return DiscordJs.ButtonBuilder.from(b).setDisabled(true);
              });

              row.setComponents(rowComponents);
