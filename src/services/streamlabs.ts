...
type NamedGroupRegex =  RegExpMatchArrayWithGroups<{voiceName: string}> | null;

class StreamLabs extends Service {
  async speak(opts: { text: string; voice: string }) {
    const match = opts.voice.match(/(?<language>.*)\s-\s(?<gender>.*)\s-\s(?<voiceName>\w+)/gi) as NamedGroupRegex;
    const response = await fetch('https://streamlabs.com/polly/speak', {
      ...
      body: JSON.stringify({
        voice: match?.groups?.voiceName ?? 'Ricardo',
        text:  opts.text,
      }),
    });
